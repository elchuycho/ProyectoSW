...
export default class AddCart extends Component{
        ...
        if (errors) {
          this.setState({ ...this.state, ...errors });
            ...
            if(cHours===0 || cHours>500){
                ...
                const uri = '/api/admin/payment/new';
                saxios.post(uri,{
    ...
    validate(state){
        ...
        if (desc !== undefined) {
            ...
            if (!longStringRegex.test(desc) || emptyRegex.test(desc)) {
              tmpErrors.push("Ingrese una descripción en un formato válido: ");
            }
            if (tmpErrors.length) {
        ...
        if (req !== undefined) {
            ...
            if (!longStringRegex.test(req) || emptyRegex.test(req)) {
              tmpErrors.push("Ingrese una cantidad de horas en un formato válido: ");
            }
            if (tmpErrors.length) {
    ...
    render(){
        const action ="Crear Método de Pago";
        const selectItems=[
